...
def mutate(genes, boardWidth, boardHeight, allPositions, nonEdgePositions):
    ...
    while count > 0:
        ...
        potentialKnightPositions = [p for positions in
                                    map(lambda x: get_attacks(x, boardWidth, boardHeight), unattacked)
                                    for p in positions if p in nonEdgePositions] if len(
            unattacked) > 0 else nonEdgePositions
...
def display(candidate, startTime, boardWidth, boardHeight):
    ...
    print("{0}\n\t{1}\t{2}".format(' '.join(map(str, candidate.Genes)), candidate.Fitness, str(timeDiff)))


def get_fitness(genes, boardWidth, boardHeight):
    attacked = set(pos for kn in genes for pos in get_attacks(kn, boardWidth, boardHeight))
    return len(attacked)
...
class KnightsTest(unittest.TestCase):
    ...
    def find_knight_positions(self, boardWidth, boardHeight, expectedKnights):
        ...
        best = genetic.get_best(fnGetFitness, None, optimalFitness, None, fnDisplay, fnMuatate, fnCreate)
